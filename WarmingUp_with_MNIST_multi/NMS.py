...
def non_max_suppression(prediction:torch.Tensor, frame_size, prob_threshold:float, iou_threshold:float)->list:
    """
    ...
    C = prediction.shape[-1]-10 

    prediction_temp = torch.concat((prediction[...,:5], prediction[...,10:]), dim=-1)
    prediction_abs_box1 = IoU.relative2absolute(prediction_temp, frame_size)
    prediction_abs_box2 = IoU.relative2absolute(prediction[...,5:], frame_size)
