...
class YoloV1(torch.nn.Module):
    def __init__(self, sizeHW, S, C, B):
        ...
        self.seq.add_module(f"conv_1", CNNBlock(3, 32, stride=2, kernel_size=7, padding=2))
        ...
        self.seq.add_module(f"conv_3", CNNBlock(128, 256, stride=1, kernel_size=3, padding=0))
        self.seq.add_module(f"maxpool_3", torch.nn.MaxPool2d(2))
        self.seq.add_module(f"conv_4", CNNBlock(256, 512, stride=1, kernel_size=3, padding=1))
        self.seq.add_module(f"maxpool_4", torch.nn.MaxPool2d(2))
        self.seq.add_module(f"conv_5", CNNBlock(512, 256, stride=1, kernel_size=1, padding=0))
        self.seq.add_module(f"conv_6", CNNBlock(256, 128, stride=1, kernel_size=3, padding=0))
        self.seq.add_module(f"conv_7", CNNBlock(128, 256, stride=1, kernel_size=3, padding=0))
        self.seq.add_module(f"conv_8", CNNBlock(256, 128, stride=1, kernel_size=3, padding=0))
        self.seq.add_module(f"conv_9", CNNBlock(128, 128, stride=1, kernel_size=3, padding=1))
