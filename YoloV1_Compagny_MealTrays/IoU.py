...
def relative2absolute_pred(prediction, cell_i, cell_j)->torch.Tensor:
    """
    ...
    Args:
        prediction (torch.Tensor of shape (TODO))
    ...
    xcr_cell, ycr_cell = prediction[N, cell_i, cell_j, 0:2].permute(1,0)
    ...
    wr_img, hr_img = prediction[N,cell_i, cell_j, 2:4].permute(1,0)
...
def relative2absolute_true(target)->tuple:
    """
    ...
    ### Retrieve box coordinates. TBM if nb_obj > 1 TODO
    xr_cell, yr_cell, wr_img, hr_img = target[N, cells_i, cells_j, 0:4].permute(1,0)
...
def intersection_over_union(target:torch.Tensor, prediction:torch.Tensor)->float:
    """
    Compute IoU between 2 boxes.
    Boxes should be [xmin, ymin, xmax, ymax, _] with absolute coordinates.

    Args:
        target (torch.Tensor of shape (N,4) or (N,5))
        prediction (torch.Tensor of shape (N,4) or (N,5))

    Return:
        iou (torch.Tensor of shape (N,1))
    """
    xmin_1, ymin_1, xmax_1, ymax_1 = target[:,:4].permute(1,0)
    xmin_2, ymin_2, xmax_2, ymax_2 = prediction[:,:4].permute(1,0)
